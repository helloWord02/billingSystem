		...
		<div class="div_content">
			...
			<div class="panel-body">
				...
				<div>
					...
					<form style="text-align: center" action="postage/update" method="post" onsubmit="return notnull()"/>
						<input type="hidden" name="id" value=${postageBean.id }>
						<span>资费名称：</span> <input
							style="margin-left: 20px; width: 200px; height: 30px; border: 1px rgba(105, 99, 70, 0.5) solid"
							placeholder="资费名称" name="postageName" type="text"
							value="${postageBean.postageName}"><br>
						<br> <span style="margin-left: -100px">资费类型：</span> 
						<select
							name="type" id="select_k1"
							style="margin-left: 20px; width: 100px; height: 30px; border: 1px rgba(105, 99, 70, 0.5) solid">
							<c:if test="${postageBean.type==1 }">
								<option value=1 selected="selected">计时</option>
								<option value=2>套餐</option>
								<option value=3>包月</option>
							</c:if>
							<c:if test="${postageBean.type==2}">
								<option value=1>计时</option>
								<option value=2 selected="selected">套餐</option>
								<option value=3>包月</option>
							</c:if>
							<c:if test="${postageBean.type==3}">
								<option value=1>计时</option>
								<option value=2>套餐</option>
						...
						<span>基本时长：</span> <input
							style="margin-left: 20px; width: 200px; height: 30px; border: 1px rgba(105, 99, 70, 0.5) solid"
							placeholder="基本时长" name="baisetime" type="text" id="baseLongTime"
							value="${postageBean.baisetime}">
						<br><br></div>
						<div id="unitPayDIV">
						<span>单位费用：</span> <input
							style="margin-left: 20px; width: 200px; height: 30px; border: 1px rgba(105, 99, 70, 0.5) solid"
							placeholder="单位费用" name="unitcost" type="text"
							value="${postageBean.unitcost}">
						<br><br></div>
						<div id="payDIV">
						 <span>基本费用：</span> <input
							style="margin-left: 20px; width: 200px; height: 30px; border: 1px rgba(105, 99, 70, 0.5) solid"
							placeholder="基本费用" name="baisecost" type="text"
							value="${postageBean.baisecost}">
							<br><br></div>
						<span style="margin-left: -100px">资费状态：</span> 
						<select
							name="state" id="select_k2"
							style="margin-left: 20px; width: 100px; height: 30px; border: 1px rgba(105, 99, 70, 0.5) solid">
							<c:if test="${postageBean.state==1}">
								<option value=1 selected="selected">暂停</option>
								<option value=2>开通</option>
							</c:if>
							<c:if test="${postageBean.state==2}">
								<option value=2>开通</option>
								<option value=3>停用</option>
							</c:if>
							<c:if test="${postageBean.state==3}">
								<option value=2>开通</option>
								<option value=3>停用</option>
							</c:if>
						</select><br>
						<br> <span>资费说明：</span> <input
							style="margin-left: 20px; width: 200px; height: 30px; border: 1px rgba(105, 99, 70, 0.5) solid"
							placeholder="资费说明" name="costexplain" type="text" value="${postageBean.costexplain}"><br>
						<br>
